...
const insertChapters = async (
  ...
  for (const { chapters, frontmatter: {language}} of books) {
    for (const { chapterDir, frontmatter: {title, omitAsChapter}, mdxContent, questions } of chapters) {
      ...
      for (const {questionId, question, type, options, answer, position} of questions) {
        await db.run(
          `
              INSERT INTO questions (chapterId, questionId, question, options, answer, questionType, lastBuildId, position)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)
