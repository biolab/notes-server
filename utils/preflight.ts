...
import {getRawCollection, RawCollectionProps} from "@/utils/getCollectionProps";
      ...
      if (args.length > 1 &&
        ...
        if (t.isObjectExpression(props)) {
          ...
          if (["multi", "text", "long-text", "choice"].indexOf(type) === -1) {
            logError(where, `Question type "${type}" is not supported. Use "multi", "text" or "long-text".`);
            return;
          }
          ...
          const newErrors: string[] = ([
            /* Add more as needed */
            [options && answer && !options.map((s) => s.toLocaleLowerCase()).includes(answer.toLocaleLowerCase()),
...
const checkBooks = async (
  ...
  Promise<Record<string, {frontmatter: ChapterFrontmatter, questions: QuestionDef[]}>> =>
{
  const chapters: Record<string, { frontmatter: ChapterFrontmatter, questions: QuestionDef[] }> = {};
  ...
  booksWithQuestions
    ...
    .forEach(({path}) => logError(path, "Books that contain questions must not be removed."));

  for (const book of books) {
    // Check that book content can be serialized
    await catchErrors(
      book.slug,
      async () => serializedContent(book.rawContent, book.frontmatter.language));

    // Check that chapters' content can be serialized
    for (const {rawContent, chapterDir} of book.chapters) {
      await catchErrors(
        chapterDir,
        async () => serializedContent(rawContent, book.frontmatter.language));
    }
    ...
    for (const {rawContent, chapterDir, frontmatter} of book.chapters) {
      const chapterQuestions = await extractQuizzes(rawContent, `${book.slug}:${chapterDir}`);
      questions.push(...chapterQuestions.map((question) => ({chapter: chapterDir, question})));
    ...
    for (const [questionId, questions] of Object.entries(questionsByIds)) {
      if (questions.length > 1) {
        logError(
          ...
          questions.map(({chapter, question: {line}}) => `- ${chapter}:${line}`).join("\n")
        );
      }
    }
    ...
    const pastQuestions = (await db.all(`
        ...
        WHERE books.path = ?`, [book.slug]) as { questionId: string }[])
      .map(({questionId}) => questionId);
    const missingQuestions = pastQuestions.filter((questionId) => !questionsByIds[questionId]);
    if (missingQuestions.length > 0) {
      missingQuestions.forEach((questionId) => {
        logError(book.slug, `Question "${questionId.slice(0, 15)} (...)" is missing.`)
      });
      const extras = questions.filter(({question: {questionId}}) => !pastQuestions.includes(questionId));
      if (extras.length > 0) {
        console.log("Hint: if any of the new questions is an edit of an existing, set its `id` to its original text.");
        extras.forEach(({chapter, question: {questionId, line}}) => {
          console.log(`- ${chapter}:${line} "${questionId.slice(0, 15)} (...)"`);
        });
        console.log();
      }
    }
...
const checkQuestions = async (
  chapters: Record<string, {frontmatter: ChapterFrontmatter, questions: QuestionDef[]}>,
  db: Database
) => {
  const questions = Object.fromEntries(
    Object.entries(chapters).map(
      ([chapterDir, {questions}]) => [
        chapterDir,
        Object.fromEntries(
  ...
  prevAnswers.forEach(({chapterId, questionId, question, answer}) => {
    ...
    if (newAnswer !== undefined && newAnswer !== answer) {
      logError(
        chapterId,
        `Question "${questionId}" (${question.slice(0, 20)}) changed the answer from "${answer}" to "${newAnswer}". This is not allowed.`
      );
    }
  });
}
...
const checkCollections = async (
  ...
  for (const collection of collections) {
    ...
    await catchErrors(
      collection.slug,
      async () => serializedContent(collection.rawContent, collection.frontmatter.language));

    // Check that all books in the collection exist
    const missingBooks = collection.books.filter(({slug}) => !allBookSlugs.has(slug));
    if (missingBooks.length > 0) {
      logError(
        collection.slug,
        `The following books are missing in the collection:\n ${missingBooks.map(b => `  ${b}`).join("\n")}`);
    }

    // Check that all collections in the collection exist
    const missingCollections = collection.collections.filter(({slug}) => !allCollectionSlugs.has(slug));
...
const insertChapters = async (
  ...
  for(const [chapterDir, {frontmatter: {title}, questions}] of Object.entries(chapters)) {
    ...
    for(const {questionId, question, type, options, answer} of questions) {
      await db.run(`
        ...
        ON CONFLICT DO UPDATE SET question = excluded.question,
                                  lastBuildId = excluded.lastBuildId`,
        [chapterId, questionId, question, JSON.stringify(options), answer, type, buildId]);
    }
  }
}
...
const insertBooks = async (
  ...
  for(const {slug: bookSlug, frontmatter: {title: bookTitle}, chapters} of books) {
...
const insertCollections = async (
  ...
  for(const {slug: collectionSlug, frontmatter: {title: collectionTitle}, books, collections: subCollections} of collections) {
...
export const prefly = async (
  ...
  const collections = await Promise.all(collectionSlugs.map(async (slug) => await getRawCollection(slug)));
